...
fn try_adopt_existing_patch(
    ...
    Ok(Some(PatchState {
        ...
        original_cave: vec![0; CAVE_BLOCK_SIZE],
        flag_addr,
...
pub fn apply_patch(mem: &dyn MemoryAccess, dos_base: usize) -> Result<PatchState> {
    ...
    if current_hook != HOOK_BYTES || current_visibility_call != VISIBILITY_CALL_BYTES {
        ...
        current_visibility_call == VISIBILITY_CALL_BYTES,
        "visibility call at CS:{VISIBILITY_CALL_OFFSET:#06x} is \
         {current_visibility_call:02X?}, expected {VISIBILITY_CALL_BYTES:02X?} — \
         already patched or unsupported game version"
    );
    ...
    if hook_readback != patched_hook || call_readback != patched_visibility_call {
        let _ = mem.write_bytes(hook_addr, &HOOK_BYTES);
        let _ = mem.write_bytes(visibility_call_addr, &VISIBILITY_CALL_BYTES);
        let _ = mem.write_bytes(cave_addr, &original_cave);
